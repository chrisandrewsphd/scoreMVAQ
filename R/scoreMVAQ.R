...
scoreMVAQ <- function(dat, verbose = 0) {
  ...
  for (i in seq(nrow(dat))) {
    ...
    theta_se_ARF[i, ] <- mirt::fscores(model4_ARF, response.pattern = mat_ARF, append_response.pattern = FALSE)
    theta_se_ACF[i, ] <- mirt::fscores(model4_ACF, response.pattern = mat_ACF, append_response.pattern = FALSE)
  }
